...
print("Starting Robust WoopSocial API Automation for All 8 Approved Videos...")
...
for item in posts:
    ...
    for attempt in range(3):
        ...
        if sess_resp.status_code in [200, 201]:
            ...
            with open(v_path, "rb") as f:
                for idx, part in enumerate(parts):
                    ...
                    if put_resp.status_code not in [200, 201, 204]:
                        print(f"  Attempt {attempt+1} chunk {idx+1} error: {put_resp.status_code}")
                        upload_ok = False
                        break
            ...
            print(f"  Attempt {attempt+1} session error: {sess_resp.status_code} {sess_resp.text}")
            time.sleep(3)
    
    if not media_id:
        print(f"Failed to upload media for {item['name']}")
        results.append({"num": item["num"], "name": item["name"], "status": "UPLOAD_FAILED"})
        continue
    ...
    if post_resp.status_code in [200, 201]:
        ...
        results.append({"num": item["num"], "name": item["name"], "status": "SCHEDULE_FAILED", "error": post_resp.text})
