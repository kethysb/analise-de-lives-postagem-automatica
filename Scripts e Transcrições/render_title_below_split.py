...
subs_dir = os.path.join(scratch_dir, "minimalist_subs_below_split")
os.makedirs(subs_dir, exist_ok=True)

frames_dir = os.path.join(scratch_dir, "frames_verification_below_split")
...
print("Building Subtitle Files for Below-Split Layout...")
for v in videos:
    v_ss = v["ss"]
    v_to = v_ss + v["t"]
...
print("Rendering 8 Videos with Title Box RIGHT BELOW THE SCREEN SPLIT (y=750)...")
...
for v in videos:
    ...
    print(f"\n[{v['num']}/8] Rendering Below-Split Title Video: {v['name']}")
    ...
    # Title Banner Positioned EXACTLY BELOW THE SPLIT (y=740, Height: 120px)
    # This sits in the wall space right above her head, exactly where the camera split begins!
    vf_chain.append("drawbox=y=740:h=120:color=black@0.85:t=fill")
    vf_chain.append(
        f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{title_text}':fontcolor=white:fontsize=42:x=(w-text_w)/2:y=780"
    )
    
    # Subtitles at Bottom (MarginV=180)
    ...
    if res.returncode == 0 and os.path.exists(out_path):
        ...
        print(f"BELOW-SPLIT SUCCESS: {v['name']} ({mb} MB)")
        
        # Extract verification frame at 3 seconds
        frame_jpg = os.path.join(frames_dir, f"v{v['num']:02d}_below_split.jpg")
        subprocess.run([
            "ffmpeg", "-y", "-ss", "3", "-i", out_path, "-vframes", "1", frame_jpg
        ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

print("\nALL 8 VIDEOS RENDERED WITH TITLE RIGHT BELOW THE SCREEN SPLIT!")
