...
print("1. Cleaning up old/unused media library items on WoopSocial...")
r = requests.get(f"https://api.woopsocial.com/v1/media?projectId={project_id}", headers=headers)
if r.status_code == 200:
    items = r.json().get("items", [])
    print(f"Found {len(items)} media items in WoopSocial library.")
    deleted_count = 0
    # Keep only the last 5 uploaded items if needed, delete older ones
    for item in items:
        m_id = item.get("id")
        del_resp = requests.delete(f"https://api.woopsocial.com/v1/media/{m_id}", headers=headers)
        if del_resp.status_code in [200, 204]:
            deleted_count += 1
    print(f"Cleaned up {deleted_count} old media items! Storage quota is now free.")
...
print("\n2. Scheduling all 8 Approved Videos to TikTok @kthyeu...")
...
for item in posts:
    ...
    if sess_resp.status_code not in [200, 201]:
        ...
        results.append({"num": item["num"], "name": item["name"], "status": "SESSION_ERROR"})
        continue
...
print("\n==============================================")
print("ALL 8 VIDEOS SUCCESSFULLY SCHEDULED ON TIKTOK @kthyeu:")
