...
subs_dir = os.path.join(scratch_dir, "minimalist_subs_facesafe")
os.makedirs(subs_dir, exist_ok=True)

frames_dir = os.path.join(scratch_dir, "frames_verification_facesafe")
os.makedirs(frames_dir, exist_ok=True)

# Generate ASS Subtitles positioned safely at the bottom (MarginV=180) below the face
...
print("Building Face-Safe Subtitle Files (MarginV=180)...")
for v in videos:
    v_ss = v["ss"]
    v_to = v_ss + v["t"]
...
print("Rendering 8 Videos with Title at Top (y=160) and Subtitles at Bottom (MarginV=180)...")
...
for v in videos:
    ...
    print(f"\n[{v['num']}/8] Rendering Face-Safe Video: {v['name']}")
    ...
    # Title Banner positioned HIGH IN THE TOP SCREEN AREA (y=160), far above her head
    vf_chain.append("drawbox=y=150:h=120:color=black@0.85:t=fill")
    vf_chain.append(
        f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{title_text}':fontcolor=white:fontsize=42:x=(w-text_w)/2:y=190"
    )
    
    # Subtitles at Bottom (MarginV=180, y=1680)
    ...
    if res.returncode == 0 and os.path.exists(out_path):
        ...
        print(f"FACE-SAFE SUCCESS: {v['name']} ({mb} MB)")
        
        # Extract verification frame at 4 seconds
        frame_jpg = os.path.join(frames_dir, f"v{v['num']:02d}_facesafe.jpg")
        subprocess.run([
            "ffmpeg", "-y", "-ss", "4", "-i", out_path, "-vframes", "1", frame_jpg
        ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

print("\nALL 8 VIDEOS RENDERED 100% FACE-SAFE WITH VERIFICATION FRAMES!")
