"""
Renderizador Bloco 2 - Titulo Redesenhado e Reposicionado
Ajustes:
- Titulo posicionado em y=480 (logo onde comeca o enquadramento da camera/rosto)
- Card editorial posicionado em y=620
- Design elegante: fundo escuro arredondado/gradiente simulado com borda fina preta, fonte destacada
- Legendas ajustadas
"""
import os
import subprocess
...
def render_video(v):
    ...
    print(f"\n[{v['num']}/9] Renderizando com Titulo Baixo & Novo Estilo: {v['name']}")
    ...
    # Construção do Filtergraph com Título Re-posicionado e Re-estilizado
    # Titulo em y=460 (faixa preta elegante semitransparente com borda sutil)
    ...
    # NOVO DESIGN DO TÍTULO (Posição y=460, altura h=90, fundo elegante dark pill com borda amarela/branca sutil)
    vf_parts.append("drawbox=y=450:h=96:color=black@0.85:t=fill")
    vf_parts.append("drawbox=y=448:h=2:color=yellow@0.8:t=fill") # Linha de topo amarela sutil
    vf_parts.append(
        f"drawtext=fontfile='C\\:/Windows/Fonts/arialbd.ttf':text='{title_text}':fontcolor=white:fontsize=34:x=(w-text_w)/2:y=480"
    )
    
    # CARD EDITORIAL (Posição y=560)
    for card in v["cards"]:
        c_txt = card["text"].replace("'", "").replace(":", "")
        cs, ce = card["s"], card["e"]
        vf_parts.append(
            f"drawbox=y=560:h=76:color=black@0.75:t=fill:enable='between(t,{cs},{ce})'"
        )
        vf_parts.append(
            f"drawtext=fontfile='C\\:/Windows/Fonts/arial.ttf':text='{c_txt}':fontcolor=yellow:fontsize=28:x=(w-text_w)/2:y=584:enable='between(t,{cs},{ce})'"
        )
...
print("=" * 60)
print("RE-RENDERIZANDO BLOCO 2 COM TÍTULOS NOVO DESIGN E POSIÇÃO (Y=450)")
print("=" * 60)
