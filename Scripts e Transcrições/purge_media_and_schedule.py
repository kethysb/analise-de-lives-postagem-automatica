...
print("1. Fetching all media items from WoopSocial storage library...")

r = requests.get("https://api.woopsocial.com/v1/media", headers=headers)
if r.status_code == 200:
    media_list = r.json().get("media", [])
    print(f"Found {len(media_list)} media items in WoopSocial library.")
    deleted_count = 0
    for item in media_list:
        m_id = item.get("id")
        del_r = requests.delete(f"https://api.woopsocial.com/v1/media/{m_id}", headers=headers)
        if del_r.status_code in [200, 204]:
            deleted_count += 1
            print(f"  Deleted media ID: {m_id}")
    print(f"Successfully purged {deleted_count} media files! Storage quota is now free.")
...
#Estudos #InteligenciaArtificial #AprenderMelhor #Psicologia""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-03T15:00:00Z"}
    },
    {
        "num": 1,
...
#InteligenciaArtificial #TrabalhoComIA #Automacao #Tecnologia""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-03T21:00:00Z"}
    },
    {
        "num": 2,
...
#InteligenciaArtificial #Automacao #Tecnologia #Produtividade""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-04T15:00:00Z"}
    },
    {
        "num": 4,
...
#CarreiraEmTecnologia #InteligenciaArtificial #Programacao #Automacao""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-04T21:00:00Z"}
    },
    {
        "num": 6,
...
#Prospeccao #VendasB2B #MarketingDigital #Empreendedorismo""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-05T15:00:00Z"}
    },
    {
        "num": 3,
...
#PromptEngineering #InteligenciaArtificial #Automacao #Produtividade""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-05T21:00:00Z"}
    },
    {
        "num": 7,
...
#AgentZero #AgentesDeIA #OpenSource #Automacao""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-06T15:00:00Z"}
    },
    {
        "num": 8,
...
#ComputerUse #AgentesDeIA #OpenSource #Automacao""",
        "schedule": {"type": "SCHEDULE_FOR_LATER", "scheduledFor": "2026-08-06T21:00:00Z"}
    }
]

print("\n2. Uploading and Scheduling all 8 Approved Videos on TikTok @kthyeu...")
...
for item in posts:
    ...
    print(f"\n==============================================")
    print(f"Uploading Video [{item['num']}]: {item['name']}")
    ...
    if sess_resp.status_code not in [200, 201]:
        ...
        results.append({"num": item["num"], "name": item["name"], "status": "SESSION_ERROR", "error": sess_resp.text})
        continue
    ...
    with open(v_path, "rb") as f:
        for idx, part in enumerate(parts):
            ...
            if put_resp.status_code not in [200, 201, 204]:
                print(f"Chunk upload error: {put_resp.status_code}")
    ...
    if comp_resp.status_code not in [200, 201]:
        ...
        results.append({"num": item["num"], "name": item["name"], "status": "COMPLETE_ERROR"})
        continue
    ...
    if post_resp.status_code in [200, 201]:
        ...
        results.append({"num": item["num"], "name": item["name"], "status": "SCHEDULE_ERROR", "error": post_resp.text})
    
    time.sleep(1)

print("\n==============================================")
print("FINAL SCHEDULING REPORT:")
